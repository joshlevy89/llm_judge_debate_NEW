...
MAX_THREADS_PER_COMBO = 200

def run_combination(args):
    """Run a single (model, debate, rep) combination."""
    model, debate_run_id, run_idx = args
    verdict_run_id = main(judge_model=model, debate_run_id=debate_run_id, max_threads=MAX_THREADS_PER_COMBO)
    return {
        "model": model,
...
if __name__ == "__main__":
    ...
    print(f"Total combinations: {total_runs} ({len(JUDGE_MODELS)} models x {len(DEBATE_RUN_IDS)} debates x {RUNS_PER_COMBINATION} reps)")
    print(f"Parallelism: {MAX_PARALLEL_PROCESSES} processes, {MAX_THREADS_PER_COMBO} threads each")
    
    start_time = time.time()
    all_runs = []
    model_progress = defaultdict(lambda: {"completed": 0, "total": len(DEBATE_RUN_IDS) * RUNS_PER_COMBINATION})
    ...
    try:
        for future in as_completed(future_to_combo):
            ...
            model, debate_run_id, run_idx = combo
            try:
                ...
                print(f"\n{'='*80}\n[BATCH] {model} | {debate_run_id} rep {run_idx} | {prog['completed']}/{prog['total']} done | verdict: {result['verdict_run_id']} | total cost: {cost_str}\n{'='*80}\n")
            except Exception as e:
                print(f"\n{'='*80}\n[BATCH ERROR] {model} | {debate_run_id} rep {run_idx} | {str(e)[:100]}\n{'='*80}\n")
